...
parser = argparse.ArgumentParser(description="Prepares the MaliciousInstruct dataset.")
parser.add_argument(
    "--persistent_dir",
    action="store",
    type=str,
...
if __name__ == "__main__":
    ...
    logging.info("Preparing MaliciousInstruct dataset:")
    log_commandline_args(args)

    with open(args.data_file_path, "r") as f:
        reader = csv.DictReader(f, fieldnames=["goal"])
        dataset = list(reader)
    ...
    logging.info(
        f"Saving {len(records)} examples to {args.persistent_dir}/data/malicious.jsonl"
    )

    os.makedirs(f"{args.persistent_dir}/data", exist_ok=True)
    with open(f"{args.persistent_dir}/data/malicious.jsonl", "w") as f:
